...
async function findReviewById(id, include) {
    if (isObjectId(id)) {
        return _database.default.fieldReview.findUnique({
            where: {
                id
            },
            include
    ...
    return _database.default.fieldReview.findFirst({
        where: {
            reviewId: id
        },
        include
...
async function updateFieldStats(fieldId) {
    // Get Fieldsy reviews stats
    const fieldsyStats = await _database.default.fieldReview.aggregate({
        where: {
            fieldId
        },
        _avg: {
    ...
    const googleStats = await _database.default.googleReview.aggregate({
        where: {
            fieldId
        },
        _avg: {
    ...
    async getFieldReviews(req, res) {
        try {
            ...
            const [fieldsyReviews, googleReviews] = await Promise.all([
                ...
                _database.default.googleReview.findMany({
                    where: {
            ...
            const transformedGoogleReviews = googleReviews.map((gr)=>({
                    id: gr.id,
                    reviewId: `G${gr.id.substring(0, 8)}`,
                    fieldId: gr.fieldId,
                    userId: null,
                    bookingId: null,
    ...
    async createReview(req, res) {
        try {
            ...
            if (bookingId) {
                ...
                completedBooking = await _database.default.booking.findFirst({
                    where: bookingWhere
                });
                if (!completedBooking) {
                ...
                const existingReviewForBooking = await _database.default.fieldReview.findFirst({
                    where: {
                ...
                const completedBookings = await _database.default.booking.findMany({
                    where: {
                        fieldId,
                        userId,
                        status: 'COMPLETED'
                ...
                const existingReviews = await _database.default.fieldReview.findMany({
                    where: {
                        bookingId: {
                            in: bookingIds
                        }
