...
async function findReviewById(id, include) {
    if (isObjectId(id)) {
        return database_1.default.fieldReview.findUnique({
            where: { id },
            include,
        });
    }
    // Search by human-readable reviewId
    return database_1.default.fieldReview.findFirst({
        where: { reviewId: id },
        include,
    });
}
...
async function updateFieldStats(fieldId) {
    ...
    const fieldsyStats = await database_1.default.fieldReview.aggregate({
        where: { fieldId },
        _avg: { rating: true },
        _count: { rating: true },
        _sum: { rating: true },
    ...
    const googleStats = await database_1.default.googleReview.aggregate({
        where: { fieldId },
        _avg: { rating: true },
        _count: { rating: true },
        _sum: { rating: true },
    ...
    async getFieldReviews(req, res) {
        try {
            ...
            const [fieldsyReviews, googleReviews] = await Promise.all([
                ...
                database_1.default.googleReview.findMany({
