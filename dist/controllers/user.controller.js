...
class UserController {
    ...
    requestEmailChange = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
        const userId = req.user.id;
        ...
        if (!newEmail) {
            throw new AppError_1.AppError('New email address is required', 400);
        }
        ...
        if (newEmail.toLowerCase() === userEmail.toLowerCase()) {
            throw new AppError_1.AppError('New email must be different from your current email', 400);
        }
        // Check if email already taken by any user (regardless of role)
        const existingUser = await user_model_1.default.findByEmail(newEmail.toLowerCase());
        if (existingUser) {
            throw new AppError_1.AppError('This email is already registered with another account', 409);
        }
        // Send OTP to the new email
        await otp_service_1.otpService.sendOtp(newEmail.toLowerCase(), 'EMAIL_CHANGE', userName);
        res.json({
            success: true,
    ...
    verifyEmailChange = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
        const userId = req.user.id;
        const userRole = req.user.role;
        const { newEmail, otp } = req.body;
        if (!newEmail || !otp) {
            throw new AppError_1.AppError('New email and verification code are required', 400);
        }
        ...
        // Verify OTP
        const isValid = await otp_service_1.otpService.verifyOtp(newEmail.toLowerCase(), otp, 'EMAIL_CHANGE');
        if (!isValid) {
            throw new AppError_1.AppError('Invalid or expired verification code', 400);
        }
        // Update the user's email
        const updatedUser = await user_model_1.default.update(userId, { email: newEmail.toLowerCase() });
        res.json({
            success: true,
