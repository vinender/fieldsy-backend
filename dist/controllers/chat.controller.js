...
const getOrCreateConversation = async (req, res) => {
    try {
        ...
        const resolvedReceiverId = await user_model_1.default.resolveId(receiverId);
        ...
        const participants = await database_1.default.user.findMany({
            where: {
                id: {
...
const getConversations = async (req, res) => {
    try {
        ...
        // Get total count
        const total = await database_1.default.conversation.count({
            where: {
                participants: {
                    has: userId
                }
...
const sendMessage = async (req, res) => {
    try {
        ...
        const [senderBlockedReceiver, receiverBlockedSender] = await Promise.all([
            database_1.default.userBlock.findUnique({
                where: {
...
const getUnreadCount = async (req, res) => {
    try {
        ...
        const unreadConversations = await database_1.default.message.findMany({
            where: {
                receiverId: userId,
                isRead: false
            },
...
const getUnreadConversationsCount = async (req, res) => {
    try {
        ...
        const conversationsWithUnread = await database_1.default.message.groupBy({
            by: ['conversationId'],
            where: {
                conversationId: {
                    in: conversationIds
