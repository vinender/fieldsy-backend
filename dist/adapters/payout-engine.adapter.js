...
Object.defineProperty(exports, "FieldsyPayoutAdapter", {
    enumerable: true,
    get: function() {
        return FieldsyPayoutAdapter;
    }
});
...
function payoutRowToRecord(p) {
    return {
        id: p.id,
        connectedAccountId: p.stripeAccountId,
...
class FieldsyPayoutAdapter {
    ...
    async findOrderById(orderId) {
        const booking = await _database.prisma.booking.findUnique({
            where: {
                id: orderId
            },
            include: {
    ...
    async findOrderByPaymentIntentId(paymentIntentId) {
        const booking = await _database.prisma.booking.findFirst({
            where: {
                paymentIntentId
            },
            include: {
    ...
    async findOrdersEligibleForPayout() {
        const bookings = await _database.prisma.booking.findMany({
            where: {
                status: {
                    in: [
                        'CONFIRMED',
    ...
    async updateOrder(orderId, data) {
        ...
        const booking = await _database.prisma.booking.update({
            where: {
                id: orderId
            },
            data: prismaData,
    ...
    async updateManyOrders(filter, data) {
        const where = {};
        if (filter.ids) where.id = {
            in: filter.ids
        };
        if (filter.subscriptionId) where.subscriptionId = filter.subscriptionId;
        if (filter.futureOnly) where.date = {
            gt: new Date()
        };
        const prismaData = orderDataToPrisma(data);
        const result = await _database.prisma.booking.updateMany({
            where,
            data: prismaData
        });
        return result.count;
    ...
    async createOrder(data) {
        const prismaData = orderDataToPrisma(data);
        const booking = await _database.prisma.booking.create({
            data: prismaData,
            include: {
                field: {
                    select: {
    ...
    async generateOrderId() {
        const counter = await _database.prisma.counter.upsert({
            where: {
                name: 'booking'
            },
            update: {
    ...
    async checkOrderAvailability(listingId, date, startTime, endTime) {
        const conflicting = await _database.prisma.booking.findMany({
            where: {
                fieldId: listingId,
                date,
                status: {
        ...
        for (const b of conflicting){
            ...
            if (reqStart >= bStart && reqStart < bEnd || reqEnd > bStart && reqEnd <= bEnd || reqStart <= bStart && reqEnd >= bEnd) {
                return {
                    available: false,
    ...
    async findConnectedAccountByUserId(userId) {
        const sa = await _database.prisma.stripeAccount.findUnique({
            where: {
                userId
            }
        });
    ...
    async findConnectedAccountByStripeId(stripeAccountId) {
        const sa = await _database.prisma.stripeAccount.findFirst({
            where: {
                stripeAccountId
            }
        });
    ...
    async createConnectedAccount(data) {
        const sa = await _database.prisma.stripeAccount.create({
            data: {
                userId: data.userId,
    ...
    async updateConnectedAccount(id, data) {
        const sa = await _database.prisma.stripeAccount.update({
            where: {
                id
            },
            data: {
    ...
    async updateConnectedAccountByStripeId(stripeAccountId, data) {
        const existing = await _database.prisma.stripeAccount.findFirst({
            where: {
                stripeAccountId
            }
        });
    ...
    async createPayout(data) {
        ...
        const payout = await _database.prisma.payout.create({
            data: prismaData
        });
        return payoutRowToRecord(payout);
    }
    async findPayoutByStripeId(stripePayoutId) {
        const payout = await _database.prisma.payout.findFirst({
            where: {
                stripePayoutId
            }
        });
    ...
    async updatePayout(id, data) {
        const prismaData = payoutDataToPrisma(data);
        const payout = await _database.prisma.payout.update({
            where: {
                id
            },
            data: prismaData
    ...
    async upsertPayoutByStripeId(stripePayoutId, data) {
        const prismaData = payoutDataToPrisma(data);
        const existing = await _database.prisma.payout.findFirst({
            where: {
                stripePayoutId
            }
        });
        if (existing) {
            const updated = await _database.prisma.payout.update({
                where: {
                    id: existing.id
                },
                data: prismaData
        ...
        const created = await _database.prisma.payout.create({
            data: {
                ...prismaData,
                stripePayoutId
            }
    ...
    async findPayoutsForAccount(connectedAccountDbId, page, limit) {
        const [payouts, total] = await Promise.all([
            _database.prisma.payout.findMany({
                where: {
                    stripeAccountId: connectedAccountDbId
                },
                skip: (page - 1) * limit,
                take: limit,
                orderBy: {
        ...
        return {
            payouts: payouts.map(payoutRowToRecord),
            total
        };
    }
    async findFailedPayouts(withinHours) {
        const since = new Date(Date.now() - withinHours * 60 * 60 * 1000);
        const payouts = await _database.prisma.payout.findMany({
            where: {
                status: 'failed',
                createdAt: {
                    gte: since
    ...
    async createTransaction(data) {
        ...
        const txn = await _database.prisma.transaction.create({
            data: prismaData
        });
        return transactionRowToRecord(txn);
    }
    async findTransactionByPaymentIntentId(stripePaymentIntentId) {
        const txn = await _database.prisma.transaction.findFirst({
            where: {
    ...
    async updateTransaction(id, data) {
        const prismaData = transactionDataToPrisma(data);
        const txn = await _database.prisma.transaction.update({
            where: {
                id
            },
            data: prismaData
    ...
    async updateTransactionsByOrderId(orderId, data) {
        const prismaData = transactionDataToPrisma(data);
        const result = await _database.prisma.transaction.updateMany({
            where: {
                bookingId: orderId
            },
            data: prismaData
    ...
    async findPendingFundsTransactions(limit) {
        const txns = await _database.prisma.transaction.findMany({
            where: {
    ...
    async findPaymentByOrderId(orderId) {
        const payment = await _database.prisma.payment.findFirst({
            where: {
                bookingId: orderId
            }
        });
    ...
    async createPayment(data) {
        const payment = await _database.prisma.payment.create({
            data: {
    ...
    async updatePayment(id, data) {
        ...
        const payment = await _database.prisma.payment.update({
            where: {
                id
            },
            data: prismaData
    ...
    async findSubscriptionById(id) {
        const sub = await _database.prisma.subscription.findUnique({
            where: {
                id
            }
        });
    ...
    async findSubscriptionByStripeId(stripeSubscriptionId) {
        const sub = await _database.prisma.subscription.findFirst({
            where: {
    ...
    async createSubscription(data) {
        ...
        const sub = await _database.prisma.subscription.create({
            data: prismaData
        });
    ...
    async updateSubscription(id, data) {
        const prismaData = subscriptionDataToPrisma(data);
        const sub = await _database.prisma.subscription.update({
            where: {
                id
            },
            data: prismaData
    ...
    async findSubscriptionsForRetry(now, maxRetries) {
        const subs = await _database.prisma.subscription.findMany({
            where: {
                status: 'past_due',
                paymentRetryCount: {
                    lt: maxRetries
    ...
    async getSystemSettings() {
        const settings = await _database.prisma.systemSettings.findFirst();
    ...
    async getListingIdsForMerchant(merchantId) {
        const fields = await _database.prisma.field.findMany({
            where: {
                ownerId: merchantId
            },
            select: {
...
//# sourceMappingURL=payout-engine.adapter.js.map