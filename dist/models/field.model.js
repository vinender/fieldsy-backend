...
class FieldModel {
    ...
    async findAll(filters) {
        ...
        const getImageScore = (field) => {
            if (!field.images || field.images.length === 0)
                return 0;
            // Check if any image is a premium URL (S3, CDN, not WordPress)
            const hasPremiumImage = field.images.some((img) => {
                if (!isValidImageUrl(img))
                    return false;
                return isPremiumImageUrl(img);
            });
            if (hasPremiumImage)
                return 2; // Premium images get highest priority
            // Check if any image is valid (including WordPress)
            const hasValidImage = field.images.some((img) => isValidImageUrl(img));
            if (hasValidImage)
                return 1; // WordPress/other valid images get medium priority
