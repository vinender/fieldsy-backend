 ...
 */ function getNextValidBookingDate(baseDate, operatingDays) {
    let nextDate = (0, _datefns.addDays)(baseDate, 1);
...
class SubscriptionService {
   ...
   */ async createSubscription({ userId, fieldId, date, timeSlot, startTime, endTime, numberOfDogs, repeatBooking, amount, paymentMethodId, customerEmail }) {
        ...
        const [user, field] = await Promise.all([
            ...
            (0, _fieldutils.resolveField)(fieldId)
        ]);
        if (!user || !field) {
        ...
        const { fieldOwnerAmount, platformCommission } = await (0, _commissionutils.calculatePayoutAmounts)(amount, field.ownerId || '');
   ...
   */ async createBookingFromSubscription(subscriptionId, bookingDate) {
        ...
        const BookingModel = (await Promise.resolve().then(()=>/*#__PURE__*/ _interop_require_wildcard(require("../models/booking.model")))).default;
        const { field } = subscription;
        // Use correct price based on booking duration
        const pricePerUnit = field.bookingDuration === '30min' ? field.price30min || field.price || 0 : field.price1hr || field.price || 0;
        // Get commission rate for this field owner
        const { commissionRate } = await (0, _commissionutils.calculatePayoutAmounts)(100, field.ownerId || '');
        // Commission rate is what platform takes as percentage
        // Determine if this is a multi-slot subscription
        const timeSlots = subscription.timeSlots && subscription.timeSlots.length > 0 ? subscription.timeSlots : [
        ...
        for (const slot of timeSlots){
            ...
            const availabilityCheck = await BookingModel.checkFullAvailability(subscription.fieldId, bookingDate, slotStart, slotEnd, undefined, subscription.id // Exclude this subscription from recurring check
            );
   ...
   */ async refundSubscriptionBookingOccurrence(bookingId, reason = 'requested_by_customer') {
        ...
        if (stripeRefund) {
            await _database.default.transaction.create({
                data: {
                    ...
                    commissionRate: booking.platformCommission && booking.totalPrice ? booking.platformCommission / booking.totalPrice * 100 : undefined,
                    type: 'REFUND',
                    status: 'COMPLETED',
                    stripeRefundId: stripeRefund.id,
    ...
    async findPaymentIntentForBooking(booking) {
        ...
        const invoices = await _stripeconfig.stripe.invoices.list({
        ...
        return typeof fallbackInvoice.payment_intent === 'string' ? fallbackInvoice.payment_intent : fallbackInvoice.payment_intent?.id || null;
    }
    /**
   ...
   */ async handleInvoicePaymentSucceeded(invoice) {
        ...
        if (subscription.interval === 'everyday') {
            ...
            nextBookingDate = getNextValidBookingDate(subscription.lastBookingDate || new Date(), operatingDays);
        ...
        if (nextBookingDate > maxFutureDate) {
            console.log(`⏭️  Next booking date (${(0, _datefns.format)(nextBookingDate, 'PPP')}) is beyond max advance booking days (${maxAdvanceBookingDays}) for subscription ${subscription.id}`);
            ...
            await (0, _notificationcontroller.createNotification)({
                ...
                message: `Your ${subscription.interval} booking payment was successful. The booking will be automatically created closer to ${(0, _datefns.format)(nextBookingDate, 'PPP')} at ${subscription.timeSlot}`,
                data: {
   ...
   */ async handleInvoicePaymentFailed(invoice) {
        ...
        const nextRetryDate = (0, _datefns.addDays)(new Date(), 1);
   ...
   */ async cancelSubscriptionDueToPaymentFailure(subscription, failureReason, totalAttempts) {
        ...
        try {
            ...
            if (subscription.user?.email) {
                _emailservice.emailService.sendSubscriptionCancelledEmail({
   ...
   */ async retryFailedPayments() {
        ...
        const subscriptionsToRetry = await _database.default.subscription.findMany({
            where: {
                status: 'past_due',
                nextRetryDate: {
                    lte: now
   ...
   */ async retrySubscriptionPayment(subscription) {
        ...
        try {
            ...
            const paidInvoice = await _stripeconfig.stripe.invoices.pay(invoice.id);
   ...
   */ async handleSubscriptionDeleted(stripeSubscription) {
        ...
        try {
            ...
            const fullSubscription = await _database.default.subscription.findUnique({
                where: {
                    id: subscription.id
                },
                include: {
   ...
   */ async cancelSubscription(subscriptionId, cancelImmediately = false) {
        ...
        if (subscription.stripeSubscriptionId && subscription.stripeSubscriptionId.startsWith('sub_')) {
            try {
                await _stripeconfig.stripe.subscriptions.update(subscription.stripeSubscriptionId, {
        ...
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const futureBookings = await _database.default.booking.findMany({
            where: {
