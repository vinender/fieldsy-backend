...
const error_notifier_service_1 = require("./services/error-notifier.service");
...
const slot_credit_expiry_job_1 = require("./jobs/slot-credit-expiry.job");
...
class Server {
    ...
    configureMiddleware() {
        ...
        this.app.use((0, cors_1.default)({
            ...
            allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'Cache-Control', 'Pragma', 'Expires', 'x-access-token'],
        ...
        // =========================================================================
        if (payout_services_1.isPayoutEngineEnabled) {
            ...
            if (engine) {
                ...
                this.app.use('/api/webhooks', engine.createWebhookRouter(express_1.default));
            }
        }
        else {
    ...
    <body>
      ...
      <script>
        ...
        this.app.use('/api/offers', offer_routes_1.default);
        this.app.use('/api/discounts', discount_routes_1.default);
        ...
        this.app.use('/api/privacy-policy', privacy_policy_routes_1.default);
        // Amenities routes
        ...
        this.app.post('/api/error-report', (req, res) => {
            ...
            (0, error_notifier_service_1.notifyError)(error, {
                type: 'API_ERROR',
                method: 'CLIENT',
                url: url || 'unknown',
                userId,
    ...
    configureErrorHandling() {
        ...
        this.app.use((err, req, res, next) => {
            ...
            if (statusCode >= 500) {
                (0, error_notifier_service_1.notifyError)(err instanceof Error ? err : new Error(err.message), {
                    type: 'API_ERROR',
                    method: req.method,
                    url: req.originalUrl,
                    userId: req.user?.id,
    ...
    start() {
        ...
        if (payout_services_1.isPayoutEngineEnabled) {
            ...
            const engine = (0, payout_services_1.getPayoutEngine)();
            if (engine)
                engine.startScheduler(node_cron_1.default);
        }
        else {
        ...
        (0, slot_credit_expiry_job_1.startSlotCreditExpiryJob)(); // Expire slot credits past validity
...
╚════════════════════════════════════════════════════╝
      `);
        ...
        process.on('uncaughtException', async (err) => {
            ...
            try {
                await (0, error_notifier_service_1.notifyError)(err, { type: 'UNCAUGHT_EXCEPTION' });
            }
            catch (_) { }
        ...
        process.on('unhandledRejection', async (err) => {
            ...
            try {
                await (0, error_notifier_service_1.notifyError)(err instanceof Error ? err : new Error(String(err)), { type: 'UNHANDLED_REJECTION' });
            }
            catch (_) { }
            if (constants_1.NODE_ENV === 'production') {
